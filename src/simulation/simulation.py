...
class Simulation:
    ...
    def __init__(self, start_time: str, duration_days: int, time_step_hours: int = 1):
        ...
        logger.info(f"Simulation Initialized: {self.start_time} to {self.end_time} (Step: {self.time_step})")

    def load_environment(self, nc_files: List[str]):
        """Load environmental data from NetCDF files."""
        if not nc_files:
            logger.warning("No environment files provided. Agents will run in 'void' mode (defaults).")
            return
            
        self.environment.load_data(nc_files)

    def create_agents(self, num_agents: int = 100, start_lat: float = 32.74, start_lon: float = -16.69):
    ...
    def run(self, max_workers: int | None = None):
        ...
        with ProcessPoolExecutor(max_workers=workers) as executor:
            while self.current_time < self.end_time:
                ...
                if step_count % 24 == 0:
                    logger.info(f"Simulation Progress: {self.current_time} ({step_count} steps). Agents: {len(self.agents)}")
    ...
    def step(self, executor):
        ...
        for agent in results:
            ...
            # Agent doesn't carry env data. We can re-query efficiently using buffers (main process).
