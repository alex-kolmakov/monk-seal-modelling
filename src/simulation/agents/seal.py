...
class SealAgent:
    ...
    def update_with_buffers(self, env_buffers: Dict):
        ...
        if self.debug and self.age_in_hours % 24 == 0:
            self.log(f"Daily Report: E={self.energy:.1f}/{self.max_energy}, Stomach={self.stomach_load:.1f}, Pos=({self.pos[0]:.2f}, {self.pos[1]:.2f})") 
        ...
        if swh > self.max_landing_swell:
             if self.state == SealState.HAULING_OUT:
                 self.state = SealState.TRANSITING # Move to safety instead of resting in impact zone 
        ...
        if self.state != old_state:
            ...
            self.log(f"State Change: {old_state.name} -> {self.state.name}. Night={is_night}, Land={is_land}, Tide={env_data.get('tide',0.5):.2f}")
    ...
    def decide_activity(self, env_data, is_night, is_land):
        ...
        if self.state == SealState.SLEEPING:
            if not is_land:
                # If we are sleeping in water (Bottling), only Haul Out if we are NOT desperate/tired
            ...
            if is_night:
                if is_land:
                    ...
                    if self.energy < self.max_energy * 0.15:
                         self.state = SealState.SLEEPING
                         self.log("Critical Energy: Sleeping in water (Bottling) instead of Hauling Out.")
                    else:
                         self.state = SealState.HAULING_OUT
                return
    ...
    def _find_nearest_land(self, env_buffers, max_radius_km=10.0, num_samples=36):
        ...
        if best_land_pos:
            self.log(f"Found land at {best_distance:.2f}km: ({best_land_pos[0]:.4f}, {best_land_pos[1]:.4f})")
    ...
    def _find_nearest_water(self, env_buffers, max_radius_km=5.0, num_samples=36):
        ...
        if best_water_pos:
            self.log(f"Found water at {best_distance:.2f}km: ({best_water_pos[0]:.4f}, {best_water_pos[1]:.4f})")
    ...
    def _move_smart(self, env_buffers, seek_land=False, target_pos=None, speed=0.05):
        ...
        if env_buffers is None:
            self.pos, self.heading = correlated_random_walk(None, self.pos, self.heading, speed=speed)
            return
        ...
        if depth_check is None or depth_check > 1000:
            ...
            for i in range(search_steps):
                for j in range(search_steps):
                    ...
                    if test_depth is not None and test_depth < 100 and not test_is_land:
                        dist = ((test_lat - self.pos[0])**2 + (test_lon - self.pos[1])**2)**0.5
                        if dist < best_dist:
                            best_dist = dist
            ...
            if best_shallow and target_pos is None:
                ...
                self.log(f"DEEP WATER PANIC! Depth={depth_check}m, heading to shallow water at {target_pos}")
            elif target_pos is None:
                # Fallback to home if no shallow water found
                bias_pos, _ = self._get_home_bias()
                if bias_pos:
                    target_pos = bias_pos
                    self.log(f"DEEP WATER PANIC! Depth={depth_check}m, no shallow water found, heading home to {target_pos}")
        ...
        for _ in range(10):
            new_pos, new_heading = correlated_random_walk(None, self.pos, self.heading, speed=step_speed)
        ...
        if currently_on_land and not seek_land:
            ...
            self.log(f"On land seeking water - allowing land crossing for escape")
        ...
        if should_check_collision:
            ...
            if blocked_count > 0:
                ...
                self.log(f"COLLISION CHECK ({land_status}): Blocked {blocked_count}/{len(candidates)} paths crossing land")
            ...
            if safe_candidates:
                ...
                self.log(f"WARNING: All paths blocked. Pos: {self.pos}, SeekLand: {seek_land}, OnLand: {currently_on_land}")
        ...
        if not seek_land and not currently_on_land and len(candidates) > 0:
            ...
            if non_coastline_candidates:
                ...
                if coastline_count > 0:
                    self.log(f"COASTLINE AVOIDANCE: Filtered out {coastline_count} coastline cells (foraging in water)")
            else:
        ...
        if seek_land:
            ...
            if land_matches:
                ...
                valid_depths = [c for c in candidates if c['data'].get('depth') is not None and c['data']['depth'] != 9999]
                if valid_depths:
                    if target_pos:
                        # BEACON FIX: If we have a target (Home), use it to break ties or guide on flat abyssal plains.
                        # Sort by depth first.
                        ...
                        best_c = min(top_k, key=lambda c: (c['pos'][0]-target_pos[0])**2 + (c['pos'][1]-target_pos[1])**2)
                    else:
                        best_c = min(valid_depths, key=lambda c: c['data'].get('depth', 9999))
                else:
                    # All are void/unknown -> fallback to random choice from all
                    if target_pos:
                         best_c = min(candidates, key=lambda c: (c['pos'][0]-target_pos[0])**2 + (c['pos'][1]-target_pos[1])**2)
                    else:
            ...
            if water_matches:
                 if target_pos:
                      ...
                      best_c = min(water_matches, key=lambda c: (c['pos'][0]-target_pos[0])**2 + (c['pos'][1]-target_pos[1])**2)
                 else:
                 ...
                 if self.memory.haulout_sites:
                      ...
                      closest_h = min(self.memory.haulout_sites, key=lambda p: (p[0]-self.pos[0])**2 + (p[1]-self.pos[1])**2)
                      ...
                      if dist_sq < 0.0025: # Very close (~0.05^2)
                           ...
                           jump_candidates = []
                           for _ in range(10):
                               jp, jh = correlated_random_walk(None, self.pos, self.heading, speed=0.15) # 3x speed
                               jd = query_env_buffers(jp[0], jp[1], env_buffers)
                               jump_candidates.append({'pos': jp, 'heading': jh, 'data': jd})
                           
                           water_jumps = [c for c in jump_candidates if not c['data'].get('is_land', False)]
                           if water_jumps:
                          ...
                          best_c = min(candidates, key=lambda c: (c['pos'][0]-closest_h[0])**2 + (c['pos'][1]-closest_h[1])**2)
    ...
    def forage(self, env_data, env_buffers):
        ...
        is_adult = self.age >= 6 # Sexual maturity is 6. Let's use 4 for "Adult Foraging"? User said "Adults... Juveniles..."
        ...
        if is_adult:
            ...
            if should_move:
                ...
                if self.distance_to_land > 12.0:
                    ...
                    self.log(f"BOUNDARY: {self.distance_to_land:.1f}km from land, returning to islands")
                    ...
                    if land_pos:
                        ...
                        if self.memory.haulout_sites:
                            target = min(self.memory.haulout_sites, 
        ...
        if actual_gain > 0:
             ...
             if depth < 100 and not final_check.get('is_land', False):
                 ...
                 self.log(f"Updated home position to current foraging location ({self.pos[0]:.4f}, {self.pos[1]:.4f})")
    ...
    def haul_out_search(self, env_buffers):
        ...
        if land_pos:
            ...
            if self.memory.haulout_sites:
                 ...
                 closest = min(self.memory.haulout_sites, key=lambda p: (p[0]-self.pos[0])**2 + (p[1]-self.pos[1])**2)
                 target = closest
                 if self.state_duration == 0:
                     self.log(f"No land found nearby, using remembered haulout site")
