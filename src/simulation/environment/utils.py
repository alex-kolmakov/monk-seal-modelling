...
def query_env_buffers(lat: float, lon: float, buffers: dict) -> dict[str, Any]:
            ...
            if key == "depth":
                ...
                if 0 <= r_idx < rows and 0 <= c_idx < cols:
                    ...
                    if original_depth_is_nan:
                        ...
                        # If less than 50% of neighbors are NaN, it's coastline
            ...
            if 0 <= r_idx < rows and 0 <= c_idx < cols:
                ...
                if np.isnan(val):
                    ...
                    if key == "depth":
                        ...
                        # Agent doesn't carry env data. Use buffers.
    ...
    if "depth" in buffers:
        if original_depth_is_nan:
            result["is_land"] = True
            # COASTLINE DETECTION: If we have inferred depth but original was NaN, it's a coastline
