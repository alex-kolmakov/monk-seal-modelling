...
def analyze_tidal_data():
    ...
    if len(valid_adt) > 0:
        ...
        if len(valid_adt) > 1:
            ...
            print(
                f"  Mean daily change: {daily_changes.mean():.4f} m "
                f"({daily_changes.mean() * 100:.2f} cm)"
            )
            print(
                f"  Max daily change: {daily_changes.max():.4f} m "
                f"({daily_changes.max() * 100:.2f} cm)"
            )
    ...
    im = ax4.imshow(
        ...
        aspect="auto",
        extent=[
            tidal.longitude.values.min(),
            tidal.longitude.values.max(),
            tidal.latitude.values.min(),
            tidal.latitude.values.max(),
        ],
    )
