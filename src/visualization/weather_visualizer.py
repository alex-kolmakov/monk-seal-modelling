...
class WeatherVisualizer:
    ...
    def create_animation(
        self, physics: xr.Dataset, waves: xr.Dataset, bgc: xr.Dataset, tidal: xr.Dataset
    ) -> Path:
        ...
        fig, axes = plt.subplots(
            2, 3, figsize=(24, 14), subplot_kw={"projection": ccrs.PlateCarree()}
        )
        axes = axes.flatten()
        ...
        p0 = axes[0].pcolormesh(
            ...
            alpha=0.8,
            vmin=self.config.temp_range[0],
            vmax=self.config.temp_range[1],
        )
        ...
        # Panel 1: Waves
        wave = (
            waves["VHM0"].isel(time=0).interp(latitude=lat, longitude=lon, method="nearest").values
        )
        p1 = axes[1].pcolormesh(
            ...
            transform=ccrs.PlateCarree(),
            alpha=0.8,
            vmin=self.config.wave_range[0],
            vmax=self.config.wave_range[1],
        )
        ...
        p2 = axes[2].pcolormesh(
            ...
            transform=ccrs.PlateCarree(),
            alpha=0.8,
            vmin=self.config.current_range[0],
            vmax=self.config.current_range[1],
        )
        ...
        p3 = axes[3].pcolormesh(
            ...
            transform=ccrs.PlateCarree(),
            alpha=0.8,
            vmin=self.config.chl_range[0],
            vmax=self.config.chl_range[1],
        )
        ...
        sea = (
            tidal["adt"]
            .isel(time=tidal_idx)
            .interp(latitude=lat, longitude=lon, method="nearest")
            .values
            * 100
        )
        p4 = axes[4].pcolormesh(
            lon, lat, sea, cmap="RdBu_r", shading="auto", transform=ccrs.PlateCarree(), alpha=0.8
        )
        ...
        anim = animation.FuncAnimation(
            fig, update, init_func=init, frames=n_frames, interval=100, blit=True
        )
